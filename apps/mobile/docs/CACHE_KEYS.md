...
3. **Rule of thumb:** a mutation must invalidate the key of _every_ cache in §1 whose
...
| `["items", "orphans", storeId]`                              | `queryKeys.items.orphans(storeId)`                                |
...
| `["aisles", "detail", id]`   | _(reserved — no reader yet)_   | —                  | —                    | —                 |
...
| `["items", "orphans", storeId]`            | Obliteration candidates (server-computed)       | `useOrphanItems`           | shopping-list rows   | 0 (+no gc) |
...
| Key                                | Contains                                                                                                        | Owner hook                        | Joins / derived from          | Stale time |
| ---------------------------------- | --------------------------------------------------------------------------------------------------------------- | --------------------------------- | ----------------------------- | ---------- |
| `["shopping-list-items", storeId]` | Shopping-list items, **joined with store-item + aisle/section names and aisle emoji, ordered by aisle/section** | `useShoppingListItems` (suspense) | `items`, `aisles`, `sections` | default    |
...
| `["notifications", "counts"]`               | Notification badge counts (pending household invitations for the current user) | `useNotificationCounts` _(no UI consumer yet)_ | 5 min      |
...
| `useBulkApplyAislesAndSections` | aisles (+ emoji) + sections (bulk)        | `["aisles", storeId]`, `["sections", storeId]`, `["items","with-details",storeId]`, `["shopping-list-items", storeId]`                        |
...
| `useMergeItems`           | folds one item into another | `["items", storeId]`, `["items","with-details",storeId]`, `["items","detail",id]`, `["items","detail",intoItemId]`, `["shopping-list-items", storeId]` |
...
| `useDeleteOrphanItems`    | many store items (unused)   | `["items", storeId]`, `["items","with-details",storeId]`, `["items","orphans",storeId]`, `["shopping-list-items", storeId]`                            |
...
| `useSwipeUpdateShoppingListItem` (`ShoppingListItem` swipe actions) | `isUnsure` / `snoozedUntil` (optimistic) | `["shopping-list-items", storeId]`                                                                                                                       |
...
| `["items", storeId]`                        | create/update/merge/get-or-create/delete item, toggle favorite, **`useUpdateAisle`** (plain list only — _not_ delete aisle), move-item-to-store (target), upsert list item, bulk import, auto-categorize, **add-recipe-to-list** (`["items"]`\*)                                                                                                           |
| `["items", "with-details", storeId]`        | create/update/merge/get-or-create/delete item, toggle favorite, update/delete aisle, `useBulkApplyAislesAndSections`, update/delete/move section, upsert list item, move-item-to-store (target), auto-categorize, **add-recipe-to-list** (`["items"]`\*)                                                                                                   |
| `["items", "detail", id]`                   | `useUpdateItem`, `useMergeItems` (both ids), `useToggleFavorite`                                                                                                                                                                                                                                                                                           |
| `["items", "orphans", storeId]`             | `useDeleteOrphanItems` (only — the preview is refetched on every open, so no other mutation needs to touch it)                                                                                                                                                                                                                                             |
| `["store-items", "search", storeId, …]`     | `useUpsertShoppingListItem`                                                                                                                                                                                                                                                                                                                                |
| `["shopping-list-items", storeId]`          | upsert/toggle/delete/remove/clear/move list item, update/delete aisle, update/delete/move/reorder section, reorder aisle, `useBulkApplyAislesAndSections`, **`useUpdateItem`**, **`useMergeItems`**, **`useDeleteItem`**, `useDeleteOrphanItems` (defensive), bulk import, auto-categorize, **`useAddRecipeToShoppingList`** (`["shopping-list-items"]`\*) |
...
- **`["items", "orphans", storeId]` cannot be derived on the client.** An item is an orphan
  only if _no_ member has it on a list, including private rows this client never receives —
  so the key is served straight from the server, with `staleTime: 0` and `gcTime: 0` because
  it gates a permanent delete. `useDeleteOrphanItems` invalidating
  `["shopping-list-items", storeId]` is **defensive only**: an orphan is on no list by
  definition, so nothing there can change. It is kept for parity with `useDeleteItem`, so a
  future loosening of the predicate does not silently strand the shopping list.
...
## 6. Server-pushed invalidation

While the Shopping List tab shows a store, `useStoreEvents` (`realtime/useStoreEvents.ts`) holds
that store's server-sent event stream open (`GET /api/stores/{storeId}/events`, opened with
`apiClient.openStream`; parsing and reconnect in `lib/realtime/storeEventStream.ts`). Events carry
**no data** — only a kind — and the client refetches through the ordinary GETs. The kind → keys
map lives in `invalidationsForKind` (`realtime/storeSync.ts`); keep this table in step with it.

| Event                                               | Refetches                                                                                                                                                                           | Marks stale only (`refetchType: "none"`)                                                         |
| --------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------ |
| `change` `list`                                     | `["shopping-list-items", storeId]`                                                                                                                                                  | `["items", storeId]`, `["items", "with-details", storeId]`, `["store-items", "search", storeId]` |
| `change` `layout`                                   | `["shopping-list-items", storeId]`, `["items", storeId]`, `["items", "with-details", storeId]`, `["aisles", storeId]`, `["sections", storeId]`, `["stores", storeId]`, `["stores"]` | —                                                                                                |
| `change` `access`                                   | `["stores"]`, `["stores", storeId]`                                                                                                                                                 | —                                                                                                |
| `event: access` / 403 / 404 (stream stops for good) | same as `change` `access`                                                                                                                                                           | —                                                                                                |
| `event: ready` (connect / reconnect)                | `["shopping-list-items", storeId]` (resync — events may have been missed)                                                                                                           | —                                                                                                |
| fallback poll, every 20 s while not connected       | `["shopping-list-items", storeId]`                                                                                                                                                  | —                                                                                                |

Rules (`startStoreSync`):

- **Coalesced**: events are debounced 300 ms per store, and keys named by several pending kinds
