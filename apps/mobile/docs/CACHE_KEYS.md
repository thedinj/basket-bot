...
| `["items", storeId]`                        | create/update/get-or-create/delete item, toggle favorite, **`useUpdateAisle`** (plain list only — _not_ delete aisle), move-item-to-store (target), upsert list item, bulk import, auto-categorize, **add-recipe-to-list** (`["items"]`\*)                       |
