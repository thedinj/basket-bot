...
export function useAddRecipeToShoppingList(
    ...
    return useTanstackMutation({
        mutationFn: ({
            ...
            return recipeApi.addToShoppingList(householdId!, recipeId!, {
                routes: included,
                factor,
            });
        },
...
export function useUpdatePlan(householdId: string | null) {
    ...
    return useTanstackMutation({
        ...
        onSuccess: (result: PlanWithDetails) => {
            queryClient.invalidateQueries({
                queryKey: queryKeys.plans.detail(householdId, result.id),
            });
            queryClient.invalidateQueries({ queryKey: queryKeys.plans.byHousehold(householdId) });
        },
        onError: (error: Error) => {
...
export function useUpdatePlanSlots(householdId: string | null) {
    ...
    return useTanstackMutation({
        ...
        onSuccess: (_, variables) => {
            queryClient.invalidateQueries({
                queryKey: queryKeys.plans.detail(householdId, variables.planId),
            });
        },
        onError: (error: Error) => {
...
export function useDispatchPlan(householdId: string | null) {
    ...
    return useTanstackMutation({
        ...
        onSuccess: (
            result: { plan: Plan; itemsAdded: number; itemsSkipped: number },
            variables
        ) => {
            queryClient.invalidateQueries({ queryKey: queryKeys.plans.byHousehold(householdId) });
            queryClient.invalidateQueries({
