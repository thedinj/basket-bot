...
$CADDY_LISTEN {
    ...
    # flush_interval -1: stream responses straight through, so the live shopping list's
    # Server-Sent Events (GET /api/stores/{id}/events) aren't held back in a proxy buffer.
    handle /api/* {
