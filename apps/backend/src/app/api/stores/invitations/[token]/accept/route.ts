...
/**
 * POST /api/stores/invitations/[token]/accept
...
export const POST = withAuth(
    async (req: AuthenticatedRequest, { params }: { params: Promise<Record<string, string>> }) => {
        try {
            ...
            storeInvitationService.acceptStoreInvitation(token, userId, userEmail);

            return NextResponse.json({ message: "Invitation accepted" }, { status: 200 });
        } catch (error: any) {
            console.error("Error accepting store invitation:", error);
