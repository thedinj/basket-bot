...
/**
 * Migration: baseline - the schema as it stood immediately before the first real migration
 * (`20260130_120000_add_checked_tracking`), taken verbatim from `src/db/init.ts` at commit
 * 1606438.
 *
 * Why this exists: without it the migration chain had no starting point. The earliest migration
 * assumed a database that already had every table, so migrations could not be replayed onto an
 * empty database at all - and `pnpm db:migrate` against a freshly `db:init`-ed database failed
 * outright with `duplicate column name`. Anchoring the chain here makes "replay every migration
 * from empty" a real operation, which is what `src/db/migrationDrift.test.ts` uses to prove that
 * `init.ts` and the migration history still agree.
 *
 * **This migration only ever runs against a genuinely empty database.** `runMigrations()` records
 * it as applied without executing it whenever application tables already exist, so no existing
 * install re-creates these tables - notably `StoreCollaborator` and `StoreInvitation`, which a
 * later migration deliberately drops.
 *
 * Being history, it is frozen: never edit it to reflect a schema change. Add a new migration.
 */
...
export function up(db: Database): void {
    ...
    console.log("  ✓ Created baseline schema");
}

export function down(db: Database): void {
    // Drops everything `up` created. Order is for readability only - the caller disables foreign
    // keys around migrations.
    db.exec(`
        DROP TABLE IF EXISTS "RefreshToken";
        DROP TABLE IF EXISTS "ShoppingListItem";
        DROP TABLE IF EXISTS "StoreItem";
        DROP TABLE IF EXISTS "StoreSection";
        DROP TABLE IF EXISTS "StoreAisle";
    ...
    console.log("  ✓ Dropped baseline schema");
}
